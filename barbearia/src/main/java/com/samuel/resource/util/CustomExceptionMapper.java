package com.samuel.resource.util;

import com.samuel.dto.ErroDto;
...
public class CustomExceptionMapper implements ExceptionMapper<ConstraintViolationException> {
	...
	public Response toResponse(ConstraintViolationException exception) {
		return Response.status(Response.Status.BAD_REQUEST).type(MediaType.APPLICATION_JSON)
				.entity(prepareErrors(exception)).build();
	}

	private List<ErroDto> prepareErrors(ConstraintViolationException exception) {
		return exception.getConstraintViolations().stream().map(this::error).collect(Collectors.toList());
	}
	...
	private String getProperty(ConstraintViolation<?> cv) {
		return cv.getPropertyPath().toString().replaceAll("([a-zA-z-09]+\\.)([a-zA-z-09]+\\.)", "");
	}
